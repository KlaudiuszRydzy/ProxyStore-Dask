...
def block_rmsd(ag, ref0, start=None, stop=None, step=None):
    ...
    return results, t_comp_final, t_all_frame, pickle_size(results)
...
def com_parallel_dask(ag, n_blocks, client):
    ...
    for iblock in range(n_blocks):
        ...
        out = delayed(block_rmsd)(block_input[0], block_input[1], start=block_input[2], stop=block_input[3], step=block_input[4])
        blocks.append(out)
    ...
    pickle_sizes = [out[3] for out in output]

    return results, t_comp_avg, t_comp_max, t_all_frame_avg, t_all_frame_max, pickle_sizes, block_input_size
            ...
            for k in traj_size:
                ...
                for i in block_size:  # changing blocks
                    for j in range(1, 3):  # fewer iterations per block size for testing
                        ...
                        start = time.time()
                        results, t_comp_avg, t_comp_max, t_all_frame_avg, t_all_frame_max, pickle_sizes, block_input_size = com_parallel_dask(mobile, i, client)
                        tot_time = time.time() - start

                        file.write("XTC{} {} {} {} {} {} {} {} {} {}\n".format(k, i, j, t_comp_avg, t_comp_max, t_all_frame_avg, t_all_frame_max, tot_time, str(pickle_sizes), block_input_size))
                        file.flush()
