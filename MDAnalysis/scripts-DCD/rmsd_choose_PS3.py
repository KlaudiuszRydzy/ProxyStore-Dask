...
def com_parallel_dask(ag, n_blocks, client, store):
    ...
    for iblock in range(n_blocks):
        ...
        proxy_out = store.proxy(out)
        blocks.append(out)

    # Proxy the entire blocks list
    block_input_size = pickle_size(blocks)
    output = client.compute(blocks, sync=True)

    # Retrieve and combine results directly from the output
    results_list = [out[0] for out in output]
...
if __name__ == "__main__":
    ...
    with Store(
        ...
        with open('data_PS3.txt', mode='w') as file:
            with performance_report(filename="report_PS3.html"):
                for k in traj_sizes:
