...
def block_rmsd(ag, ref0, start=None, stop=None, step=None):
    ...
    return results, t_comp_final, t_all_frame, size
...
def com_parallel_dask(ag, n_blocks, client):
    ...
    for iblock in range(n_blocks):
        ...
        out = delayed(block_rmsd)(ag, ref0, start, stop, step)
        if block_input_size is None:
            block_input_size = pickle_size(out)
        blocks.append(out)
    ...
    pickle_size_result = output[0][3]

    return results, t_comp_avg, t_comp_max, t_all_frame_avg, t_all_frame_max, pickle_size_result, block_input_size

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run MDAnalysis benchmarks with Dask.')
        ...
        with performance_report(filename="report.html"):
            for k in traj_sizes:
                ...
                for j in range(iterations):  # number of iterations
                    ...
                    start = time.time()
                    results, t_comp_avg, t_comp_max, t_all_frame_avg, t_all_frame_max, pickle_size_result, block_input_size = com_parallel_dask(mobile, block_size, client)
